import { parseVoucherStat } from '@/lib/shopee/parseVoucherStat'
...
export async function POST(req: NextRequest) {
  ...
  // 파일명 prefix 검증
  if (!file.name.toLowerCase().startsWith('voucher')) {
    return NextResponse.json(
      { error: '파일명이 "voucher" 로 시작해야 합니다.' },
      { status: 400 },
    )
  }
  ...
  const result = await parseVoucherStat(
    fileBuffer,
    shopeeAccountId,
    account.brand_id,
    country,
