import { parseProductPerformance } from '@/lib/shopee/parseProductPerformance'
...
export async function POST(req: NextRequest) {
  ...
  if (!yearMonth || !/^\d{4}-(0[1-9]|1[0-2])$/.test(yearMonth)) {
    return NextResponse.json({ error: '월(YYYY-MM) 정보가 올바르지 않습니다.' }, { status: 400 })
  }
  ...
  // 파일명 prefix 검증
  if (!file.name.toLowerCase().startsWith('parentskudetail')) {
    return NextResponse.json(
      { error: '파일명이 "parentskudetail" 로 시작해야 합니다.' },
      { status: 400 },
    )
  }
  ...
  const result = await parseProductPerformance(
    fileBuffer,
    shopeeAccountId,
    account.brand_id,
    country,
