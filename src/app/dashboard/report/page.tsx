  ...
  if (role === 'admin') {
    ...
    if (myBrandIds.length > 0) {
      const { data: reportsData } = await supabase
        .from('reports')
        .select(
          'id, brand_id, title, platform, country, internal_account_id, year, month, status, insight_memo, insight_memo_gmv_max, created_by, created_at, updated_at, brands(name)',
        )
